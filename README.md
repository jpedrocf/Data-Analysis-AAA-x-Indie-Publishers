...
# Support Material and Data Preprocessing

A [Data Catalog](DataCatalog.md) was established to enhance understanding of the dataset metrics, providing clear definitions and descriptions for each attribute. 

All the graphs, insights, data processing, and treatment were generated by code in the [Notebook (Google Colab)](MVPDataScienceGamesOrdered.ipynb). The process began by loading the dataset and checking the first rows to ensure the data was properly imported. Next, a check for null values was conducted, and any missing data or zeros was replaced with the mean, except for the price column, where zero values were deemed acceptable (as they represent free games). Dataframes were created to better organize the information, followed by a comparative analysis of the top 5 values to extract key insights. A Pearson correlation analysis was performed to assess relationships between variables, and various graphical comparisons were made to visualize the data, incorporating outliers to enhance insights.
...
After these analyses, a second round of data cleaning took place, specifically aimed at preparing the dataset for machine learning. This included creating the variable playtimePerDay to account for potential bias in the avgPlaytime variable during the machine learning process. Games that were released recently have had less time to accumulate average playtime, which could skew the results. Additionally, outliers were identified and removed, which was done after generating the graphs, as the outliers contributed to valuable insights earlier in the process. Once the outliers were removed, the data was normalized, leading into the final steps of training and evaluating the machine learning model.

During the model training phase, I first separated the features (X) and the target variable (y), where the target was the reviewScoreClass, a classification label I created by categorizing the game's review scores. The data was split into training and test sets using an 80%-20% split to ensure a robust evaluation of the model's performance on unseen data. Before training, the feature set was scaled to normalize the data, enhancing the performance and convergence of the algorithm.
